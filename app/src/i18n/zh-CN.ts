export default {
  ...
  settings: {
    ...
    privacy: {
      ...
      userRulesDesc: '在可视化编辑器中维护规则,优先级高于内置 19 条 bundle id / 12 条路径规则。保存后即时生效,无需重启。',
      userRulesEditButton: '打开规则编辑器',
      userRulesPathLabel: '规则文件位置 (高级)',
      ...
      userRulesEmptyHint: '尚未添加用户规则,只走 builtin 规则。点上方「打开规则编辑器」开始添加。',
    ...
    scanOptions: {
      ...
      detectDuplicates: '扫描后自动检测重复文件',
      detectDuplicatesDesc: '扫描完成后,自动用 BLAKE3 双阶段哈希(大小分组 → 头部 64KB → 全文)在结果中找出字节级相同的文件,结果在 Scan 页「重复文件」Tab 展示。',
      aiTagging: '扫描后自动 AI 打标签',
      aiTaggingDesc: '扫描完成后,自动给 ≥100 MB 的中/高风险文件批量补打 AI 标签(批量请求降低 token 消耗)。',
    },
    userRulesEditor: {
      title: '用户规则编辑器',
      desc: '在此添加、编辑、删除分类规则。这些规则优先级高于内置 builtin 规则。保存后立即生效。',
      loading: '正在读取规则文件…',
      loadFailed: '读取规则失败',
      saveOk: '已保存 {n} 条规则,即时生效',
      saveFailed: '保存失败',
      saveBlockedFields: '请检查所有字段:id、category、reason_key 必填,matcher 不能有空值',
      saveBlockedDupId: '存在重复的规则 id,请修改成唯一值',
      empty: '当前没有任何用户规则。点下方「添加规则」开始。',
      addRule: '添加规则',
      removeRule: '删除此规则',
      unnamedRule: '(未命名)',
      fieldId: '规则 ID',
      placeholderId: '例如 my_dropbox_cache (字母 / 数字 / 下划线)',
      fieldCategory: '分类 (写入 scan_result.category)',
      placeholderCategory: '例如 browser_cache / dev_artifacts',
      fieldRisk: '风险等级',
      fieldReasonKey: '说明 i18n key (写入 ai_reason)',
      placeholderReasonKey: 'classifier.reason.xxx (前端 i18n 字典里要有这个 key)',
      matcherSection: '匹配规则',
      fieldValue: '路径子串 (小写比较)',
      placeholderValue: '例如 /library/caches/com.dropbox.dropbox',
      fieldValues: '路径子串列表 (任意命中即视为匹配)',
      addValue: '添加一项',
      fieldExts: '扩展名列表 (不含点)',
      placeholderExt: '例如 mp4',
      addExt: '添加扩展名',
      fieldSizeBytes: '阈值 (字节)',
      warnDupId: '存在重复的 id,无法保存',
      risk: {
        low: '低',
        medium: '中',
        high: '高',
      ...
      matcherKind: {
        path_contains: '路径包含子串',
        path_contains_any: '路径包含任意一项',
        path_ends_with: '路径以…结尾',
        ext_in_and_size_gt: '扩展名 + 大小 ≥ 阈值',
        size_gte: '大小 ≥ 阈值',
      },
    },
    providers: {
