export default {
  ...
  settings: {
    ...
    privacy: {
      ...
      userRulesDesc: 'Manage rules in the visual editor. They take precedence over the 19 builtin bundle-id rules and 12 path rules. Saved changes take effect immediately — no restart required.',
      userRulesEditButton: 'Open rule editor',
      userRulesPathLabel: 'Rule file location (advanced)',
      ...
      userRulesEmptyHint: 'No user rules yet — only builtin rules are active. Click "Open rule editor" above to add your first rule.',
    ...
    scanOptions: {
      ...
      detectDuplicates: 'Auto-detect duplicates after scan',
      detectDuplicatesDesc: 'After each scan, automatically runs the two-stage BLAKE3 detector (size group → head 64KB → full file) to find byte-identical files. Results appear in the "Duplicates" tab of the Scan page.',
    ...
    userRulesEditor: {
      ...
      desc: 'Add, edit, or remove classifier rules here. They take precedence over the builtin set. Changes are applied immediately on save.',
      loading: 'Loading rule file…',
      loadFailed: 'Failed to read rules',
      saveOk: 'Saved {n} rule(s) — active immediately',
      saveFailed: 'Save failed',
      saveBlockedFields: 'Please check all fields: id, category, and reason_key are required; matcher inputs cannot be empty',
      saveBlockedDupId: 'Duplicate rule id detected — please make ids unique',
      empty: 'No user rules yet. Click "Add rule" below to get started.',
      addRule: 'Add rule',
      removeRule: 'Delete this rule',
      unnamedRule: '(unnamed)',
      fieldId: 'Rule ID',
      placeholderId: 'e.g. my_dropbox_cache (letters / digits / underscores)',
      fieldCategory: 'Category (writes scan_result.category)',
      placeholderCategory: 'e.g. browser_cache / dev_artifacts',
      fieldRisk: 'Risk level',
      fieldReasonKey: 'Reason i18n key (writes ai_reason)',
      placeholderReasonKey: 'classifier.reason.xxx (must exist in i18n dict)',
      matcherSection: 'Matcher',
      fieldValue: 'Path substring (lowercase compare)',
      placeholderValue: 'e.g. /library/caches/com.dropbox.dropbox',
      fieldValues: 'Path substrings (any match counts as a hit)',
      addValue: 'Add value',
      fieldExts: 'Extension list (no leading dot)',
      placeholderExt: 'e.g. mp4',
      addExt: 'Add extension',
      fieldSizeBytes: 'Threshold (bytes)',
      warnDupId: 'Duplicate rule id — cannot save',
      risk: {
        low: 'Low',
        medium: 'Medium',
        high: 'High',
      ...
      matcherKind: {
        path_contains: 'Path contains substring',
        path_contains_any: 'Path contains any of',
        path_ends_with: 'Path ends with',
        ext_in_and_size_gt: 'Extension + size ≥ threshold',
        size_gte: 'Size ≥ threshold',
      },
    },
    providers: {
