...
//   - isExpanded / toggleExpand 受控,展开状态由顶层 Set<string> 单一来源
//   - 配合 useVirtualizer:5000 节点也只创建可见 ~20 个组件实例
...
function toggle() {
  ...
  emit('toggleExpand', props.node.fullPath || props.node.name)
}
...
<template>
  <div
    class="group grid h-full grid-cols-[40px_minmax(0,1fr)_100px_72px_80px] items-center gap-2 border-b py-2 pr-2 text-sm transition-colors hover:bg-accent/40 md:grid-cols-[40px_minmax(0,1fr)_110px_100px_72px_80px]"
  >
    ...
    >
      ...
      <component
        :is="node.isFile ? FileText : (isExpanded ? FolderOpen : Folder)"
